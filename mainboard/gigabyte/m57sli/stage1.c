...
//		PCM(0, 0x18, 1, 0xB8 , 0x000000f0, 0x00fc0003),
		 ...
		 * [31:25] Reserved
		 */
//		PCM(0, 0x18, 1, 0xC4 , 0xFE000FC8, 0x00007000),
		 ...
		 * [31:25] Reserved
		 */
//		PCM(0, 0x18, 1, 0xC0 , 0xFE000FCC, 0x00000033),
		 ...
		 */
//		PCM(0, 0x18, 1, 0xE0 , 0x0000FC88, 0xff000003), /* link 0 of cpu 0 --> Nvidia MCP55 */
...
void hardware_stage1(void)
{
	...
	rawpnp_enter_ext_func_mode(SUPERIO_DEV);
	/* The following line will set CLKIN to 24 MHz, external */
	rawpnp_write_config(SUPERIO_DEV, IT8716F_CONFIG_REG_CLOCKSEL, 0x11);
	tmp = rawpnp_read_config(SUPERIO_DEV, IT8716F_CONFIG_REG_SWSUSP);
	/* Is serial flash enabled? Then enable writing to serial flash. */
	if (tmp & 0x0e) {
		rawpnp_write_config(SUPERIO_DEV, IT8716F_CONFIG_REG_SWSUSP, tmp | 0x10);
		rawpnp_set_logical_device(SUPERIO_DEV, GPIO_DEV);
		/* Set Serial Flash interface to 0x0820 */
		rawpnp_write_config(SUPERIO_DEV, 0x64, 0x08);
		rawpnp_write_config(SUPERIO_DEV, 0x65, 0x20);
		/* We can get away with not resetting the logical device because
		 * it8716f_enable_serial will do that.
		 */
	}
 	it8716f_enable_serial(SUPERIO_DEV, SERIAL_DEV, SERIAL_IOBASE);
	rawpnp_exit_ext_func_mode(SERIAL_DEV);
