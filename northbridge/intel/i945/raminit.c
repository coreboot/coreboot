...
static void do_ram_command(u32 command)
{
	...
	printk(BIOS_DEBUG,"   Sending RAM command 0x%08x", reg32);
...
static void sdram_detect_errors(void)
{
	...
	if (reg8 & ((1<<7)|(1<<2))) {
		...
		reg8 = pci_conf1_read_config8(PCI_DEV(0, 0x1f, 0), 0xa4); /* GEN_PMCON_3 */
		reg8 |= (1 << 3);
...
static void sdram_detect_cas_latency_and_ram_speed(struct sys_info * sysinfo, u8 cas_mask)
{
	...
	printk(BIOS_DEBUG,"lowest common cas = %d\n", lowest_common_cas);

	for (j = max_ram_speed; j>=0; j--) {
		int freq_cas_mask = cas_mask;

		printk(BIOS_DEBUG,"Probing Speed %d\n", j);
		for (i=0; i<2*DIMM_SOCKETS; i++) {
			...
			while (current_cas_mask) {
				...
				printk(BIOS_DEBUG,"    Current CAS mask: %04x; ", current_cas_mask);
				...
				printk(BIOS_DEBUG,"idx=%d, ", idx);
				printk(BIOS_DEBUG,"tCLK=%x, ", spd_read_byte(DIMM_SPD_BASE + i, spd_lookup_table[2*idx]));
				printk(BIOS_DEBUG,"tAC=%x", spd_read_byte(DIMM_SPD_BASE + i, spd_lookup_table[(2*idx)+1]));
				...
				printk(BIOS_DEBUG,":    Not fast enough!\n");
			...
			if (!current_cas_mask) {
				printk(BIOS_DEBUG,"    No valid CAS for this speed on DIMM %d\n", i);
				break;
			}
		}
		printk(BIOS_DEBUG,"  freq_cas_mask for speed %d: %04x\n", j, freq_cas_mask);
		if (freq_cas_mask) {
			switch (j) {
...
#define CDCLK_320MHz	0x40
	...
	if (freq != CRCLK_400MHz) {
		...
		reg8 = (pci_conf1_read_config8(PCI_DEV(0, 0x00,0), 0xe7) & 0x70) >> 4;
		if (reg8==2)
			freq = CRCLK_166MHz;
	}
	...
	if (second_vco) {
		printk(BIOS_DEBUG, "Programming second TCO\n");
...
static void sdram_program_clock_crossing(void)
{
	...
	switch ((reg32 >> 4) & 7) {
	case 2:	printk(BIOS_DEBUG, "400"); idx += 0; break;
	case 3:	printk(BIOS_DEBUG, "533"); idx += 2; break;
	case 4:	printk(BIOS_DEBUG, "667"); idx += 4; break;
	default: printk(BIOS_DEBUG, "RSVD\n"); return;
	}

	printk(BIOS_DEBUG, " FSB=");
	switch (reg32 & 7) {
	case 0:	printk(BIOS_DEBUG, "400\n"); return;
	case 1:	printk(BIOS_DEBUG, "533"); idx += 0; break;
	case 3:	printk(BIOS_DEBUG, "667"); idx += 4; break;
	default: printk(BIOS_DEBUG, "RSVD\n"); return;
	}
...
{
	printk(BIOS_DEBUG,"Normal Operation\n");
