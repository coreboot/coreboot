	...
	unsigned long apicaddr)
{
	...
	printk(BIOS_DEBUG, "writing IOAPIC: ID %d, ver %02x, addr %lx\n", id, ver, apicaddr);
...
{
	printk(BIOS_DEBUG, "smp_write_intsrc irqtype %x, irqflag %x, srcbus %d, srcbusirq %d, dstapic %x, dstirq %x\n",
	irqtype, irqflag, srcbus, srcbusirq, dstapic, dstirq);
