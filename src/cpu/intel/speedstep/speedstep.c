...
static void speedstep_get_limits(sst_params_t *const params)
{
	...
	case 0x1067:
		...
		params->turbo.power	= SPEEDSTEP_TURBO_POWER_PENRYN;
		break;
	default:
		...
		params->turbo.power	= SPEEDSTEP_TURBO_POWER_MEROM;
		break;
	}
}
...
void speedstep_gen_pstates(sst_table_t *const table)
{
	...
	const int power_step	= power_diff2 / ratio_diff2;
	const int vid_step	= vid_diff2 / ratio_diff2;
