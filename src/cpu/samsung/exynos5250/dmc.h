 ...
 * field with 2 possible values and packing it with, say, 2 bits. This
 * works fine until it doesn't, and when it doesn't is just want you
 * want a robust encoding. So, let's be a bit smart here. First, yes
 * yes yes it's clever to let the enum count entries for you, except
 * in the common case when there are two of them, and a quick scan
 * reveals that fact. So let's go ahead and do the really hard part of
 * counting them. And, let's set the values to something we can easily
 * scan for in memory. Since '1' and '2' are rather common, we pick
 * something that's actually of some value when things go south.
 * This setup motivated by a use case: something's going wrong and
 * having a manuf name of '1' or '2' is completely useless!
 */
enum mem_manuf {
