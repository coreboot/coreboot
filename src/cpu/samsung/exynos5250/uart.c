...
static void serial_setbrg_dev(unsigned base_port)
{
//	struct s5p_uart *const uart = s5p_get_base_uart(dev_index);
	...
	case EXYNOS5_UART0_BASE:
		periph = PERIPH_ID_UART0;
		break;
	case EXYNOS5_UART1_BASE:
		periph = PERIPH_ID_UART1;
		break;
	case EXYNOS5_UART2_BASE:
		periph = PERIPH_ID_UART2;
		break;
	case EXYNOS5_UART3_BASE:
		periph = PERIPH_ID_UART3;
		break;
	default:
		return;
	}
 ...
 */
static void exynos5_init_dev(unsigned base_port)
{
//	struct s5p_uart *const uart = s5p_get_base_uart(dev_index);
	struct s5p_uart *uart = (struct s5p_uart *)base_port;
	...
	serial_setbrg_dev(base_port);
}

static int exynos5_uart_err_check(unsigned base_port, int op)
{
	//struct s5p_uart *const uart = s5p_get_base_uart(dev_index);
	struct s5p_uart *uart = (struct s5p_uart *)base_port;
	unsigned int mask;
 ...
 */
static unsigned char exynos5_uart_rx_byte(unsigned base_port)
{
//	struct s5p_uart *const uart = s5p_get_base_uart(dev_index);
	struct s5p_uart *uart = (struct s5p_uart *)base_port;
	...
	while (!(readl(&uart->ufstat) & (RX_FIFO_COUNT_MASK |
					 RX_FIFO_FULL_MASK))) {
		if (exynos5_uart_err_check(base_port, 0))
			return 0;
	}
...
static void exynos5_uart_tx_byte(unsigned base_port, unsigned char data)
{
//	struct s5p_uart *const uart = s5p_get_base_uart(dev_index);
	struct s5p_uart *uart = (struct s5p_uart *)base_port;
	...
	while ((readl(uart->ufstat) & TX_FIFO_FULL_MASK)) {
		if (exynos5_uart_err_check(base_port, 1))
			return;
	}
...
void uart_init(unsigned base_port)
{
	exynos5_init_dev(base_port);
}

unsigned char uart_rx_byte(unsigned base_port)
{
	return exynos5_uart_rx_byte(base_port);
}

void uart_tx_byte(unsigned base_port, unsigned char data)
{
	exynos5_uart_tx_byte(base_port, data);
}
#endif
