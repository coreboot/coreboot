...
unsigned int nano_update_ucode(void)
{
	...
	for( i = 0; i < (ucode_len >> 2); /* don't increment i here */)
	{
		...
		/* Since we have a valid microcode, there's no need to serach
...
#if 0
	/* FIXME: This loop will most likely go away, but I will save it for a
	 * while. It won't be in the final patch, I promise. */
