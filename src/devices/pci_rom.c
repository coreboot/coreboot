...
struct rom_header *pci_rom_probe(struct device *dev)
{
	...
	if ((dev->class >> 8) == PCI_CLASS_DISPLAY_VGA)
		rom_header = cbfs_load_optionrom(dev->vendor,
			(dev->device & CONFIG_VGA_BIOS_DEVICEID_MASK), NULL);
	else
	...
	if ((dev->class >> 8) == PCI_CLASS_DISPLAY_VGA) {
		if (dev->vendor != rom_data->vendor
		    || ((dev->device & CONFIG_VGA_BIOS_DEVICEID_MASK) !=
			(rom_data->device & CONFIG_VGA_BIOS_DEVICEID_MASK))) {
			printk(BIOS_ERR, "ID mismatch: vendor ID %04x, "
			       "device ID %04x\n", rom_data->vendor,
			       rom_data->device);
			return NULL;
		}
	} else if (dev->vendor != rom_data->vendor
