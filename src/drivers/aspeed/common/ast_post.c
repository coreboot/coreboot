...
u32 ast_mindwm(struct ast_private *ast, u32 r)
{
	...
	do {
		...
		timeout++;
	} while ((data != (r & 0xffff0000)) && (timeout < COREBOOT_AST_FAILOVER_TIMEOUT));
...
static void ast_init_dram_reg(struct drm_device *dev)
{
	...
	if ((j & 0x80) == 0) { /* VGA only */
		if (ast->chip == AST2000) {
			...
			do {
				timeout++;
			} while ((ast_read32(ast, 0x10100) != 0xa8) && (timeout < COREBOOT_AST_FAILOVER_TIMEOUT));
			if (timeout >= COREBOOT_AST_FAILOVER_TIMEOUT)
				dev_err(dev->pdev, "Timeout while waiting for register\n");
		} else {/* AST2100/1100 */
...
ddr3_init_start:
	...
	do {
		...
		timeout++;
	} while ((!(data & 0x08000000)) && (timeout < COREBOOT_AST_FAILOVER_TIMEOUT));
	if (timeout >= COREBOOT_AST_FAILOVER_TIMEOUT)
		dev_err(dev->pdev, "Timeout while waiting for register\n");
	data = ast_mindwm(ast, 0x1E6E001C);
	data = (data >> 8) & 0xff;
	timeout = 0;
	while (((data & 0x08) || ((data & 0x7) < 2) || (data < 4)) && (timeout < COREBOOT_AST_FAILOVER_TIMEOUT)) {
		...
		do {
			data = ast_mindwm(ast, 0x1E6E001C);
			timeout2++;
		} while ((!(data & 0x08000000)) && (timeout2 < COREBOOT_AST_FAILOVER_TIMEOUT));
		if (timeout2 >= COREBOOT_AST_FAILOVER_TIMEOUT)
			dev_err(dev->pdev, "Timeout while waiting for register\n");
	...
	do {
		...
		timeout++;
	} while ((!(data & 0x00001000)) && (timeout < COREBOOT_AST_FAILOVER_TIMEOUT));
	if (timeout >= COREBOOT_AST_FAILOVER_TIMEOUT)
		dev_err(dev->pdev, "Timeout while waiting for register\n");
	ast_moutdwm(ast, 0x1E6E0070, 0x00000000);
	ast_moutdwm(ast, 0x1E6E0050, 0x80000000);
...
static void ast_post_chip_2300(struct drm_device *dev)
{
	...
	if ((reg & 0x80) == 0) {/* vga only */
		...
		do {
			timeout++;
		} while ((ast_read32(ast, 0x12000) != 0x1) && (timeout < COREBOOT_AST_FAILOVER_TIMEOUT));
		if (timeout >= COREBOOT_AST_FAILOVER_TIMEOUT)
			dev_err(dev->pdev, "Timeout while waiting for register\n");
		...
		do {
			timeout++;
		} while ((ast_read32(ast, 0x10000) != 0x1) && (timeout < COREBOOT_AST_FAILOVER_TIMEOUT));
		if (timeout >= COREBOOT_AST_FAILOVER_TIMEOUT)
			dev_err(dev->pdev, "Timeout while waiting for register\n");
