		...
		u32 offset, size_t len, void *buf, u8 command)
{
	...
	if (sn->params->addr_len == 3 && sn->params->merge_cmd == 0) {
		...
		printk(BIOS_WARNING, "SF: unsupported addr_len or merge_cmd\n");
		return -1;
	}
	...
	if (ret < 0)
		printk(BIOS_WARNING, "SF: Transaction failed\n");
...
struct spi_flash *spi_fram_probe_ramtron(struct spi_slave *spi, u8 *idcode)
{
	...
	case 0xff:
		...
		printk(BIOS_WARNING, "SF: Unsupported non-JEDEC RAMTRON device "
	...
	printk(BIOS_WARNING, "SF/ramtron: unsupported device id0=%02x id1=%02x id2=%02x\n",
