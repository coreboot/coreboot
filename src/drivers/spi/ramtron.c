		...
		u32 offset, size_t len, void *buf, u8 command)
{
	...
	if (sn->params->addr_len == 3 && sn->params->merge_cmd == 0) {
		...
		printk(BIOS_SPEW, "SF: unsupported addr_len or merge_cmd\n");
		return -1;
	}
	...
	if (command == CMD_RAMTRON_WRITE) {
		...
		if (ret < 0) {
			printk(BIOS_SPEW, "SF: Enabling Write failed\n");
			goto releasebus;
		}
	}
	...
	if (ret < 0)
		printk(BIOS_SPEW, "SF: Transaction failed\n");
...
{
	printk(BIOS_SPEW, "SF: Erase of RAMTRON FRAMs is pointless\n");
	return -1;
}
...
struct spi_flash *spi_fram_probe_ramtron(struct spi_slave *spi, u8 *idcode)
{
	...
	case 0xff:
		...
		printk(BIOS_SPEW, "SF: Unsupported non-JEDEC RAMTRON device "
	...
	printk(BIOS_SPEW, "SF/ramtron: unsupported device id0=%02x id1=%02x id2=%02x\n",
...
found:
	sn = malloc(sizeof(*sn));
	if (!sn) {
		printk(BIOS_SPEW, "SF: Failed to allocate memory\n");
		return NULL;
	}
