...
int spi_flash_cmd(struct spi_slave *spi, u8 cmd, void *response, size_t len)
{
	...
	if (ret)
		printk(BIOS_SPEW, "SF: Failed to send command %02x: %d\n", cmd, ret);
		...
		size_t cmd_len, void *data, size_t data_len)
{
	...
	if (ret) {
		printk(BIOS_SPEW, "SF: Failed to send read command (%zu bytes): %d\n",
				data_len, ret);
	}
		...
		const void *data, size_t data_len)
{
	...
	if (ret) {
		printk(BIOS_SPEW, "SF: Failed to send write command (%zu bytes): %d\n",
				data_len, ret);
	}
			...
			u32 offset, size_t len)
{
	...
	if (offset % erase_size || len % erase_size) {
		printk(BIOS_SPEW, "SF: Erase offset/length not multiple of erase size\n");
		return -1;
	}

	ret = spi_claim_bus(flash->spi);
	if (ret) {
		printk(BIOS_SPEW, "SF: Unable to claim SPI bus\n");
		return ret;
	}
	...
	printk(BIOS_SPEW, "SF: Successfully erased %zu bytes @ %#x\n", len, start);
		...
		unsigned int max_hz, unsigned int spi_mode)
{
	...
	if (!spi) {
		printk(BIOS_SPEW, "SF: Failed to set up slave\n");
		return NULL;
	}

	ret = spi_claim_bus(spi);
	if (ret) {
		printk(BIOS_SPEW, "SF: Failed to claim SPI bus: %d\n", ret);
		goto err_claim_bus;
	}
	...
	printk(BIOS_SPEW, "SF: Detected %s with page size %x, total %x\n", flash->name, flash->sector_size, flash->size);
