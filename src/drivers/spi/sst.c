...
sst_disable_writing(struct spi_flash *flash)
{
	...
	if (ret)
		printk(BIOS_SPEW, "SF: Disabling Write failed\n");
	return ret;
}
...
sst_write(struct spi_flash *flash, u32 offset, size_t len, const void *buf)
{
	...
	for (; actual < len - 1; actual += 2) {
		...
		ret = spi_flash_cmd_write(flash->spi, cmd, cmd_len,
		                          buf + actual, 2);
		if (ret) {
			printk(BIOS_SPEW, "SF: sst word program failed\n");
			break;
		}
...
sst_unlock(struct spi_flash *flash)
{
	...
	if (ret)
		printk(BIOS_SPEW, "SF: Unable to set status byte\n");
...
spi_flash_probe_sst(struct spi_slave *spi, u8 *idcode)
{
	...
	if (i == ARRAY_SIZE(sst_spi_flash_table)) {
		printk(BIOS_SPEW, "SF: Unsupported SST ID %02x\n", idcode[1]);
		return NULL;
	}
