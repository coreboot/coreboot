...
static void h8_log_ec_version(void)
{
	...
	memset(ecfw, 0, sizeof ecfw);
	h8_build_id_and_function_spec_version(ecfw, sizeof ecfw);
	ecfw[sizeof ecfw - 1] = 0;
...
u8 h8_build_id_and_function_spec_version(char *buf, u8 buf_len)
{
	...
	i += sprintf(str + i, "-%d.%d", ec_read(0xef), ec_read(0xeb));

	memcpy(buf, str, MIN(buf_len, i));
