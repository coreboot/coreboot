...
struct nic *sis900_probe(struct nic *nic, unsigned short *io_addrs, struct pci_dev *pci);
...
static int sis900_get_mac_addr(struct pci_dev * pci_dev , struct nic *nic)
{
	u16 signature;
	int i;
	...
	if (signature == 0xffff || signature == 0x0000) {
		printk_err ("sis900_probe: Error EERPOM read %x\n", signature);
		return 0;
	}
 ...
 */
static int sis630e_get_mac_addr(struct pci_dev * pci_dev, struct nic *nic)
{
	u8 reg;
	int i;
	...
	isa_bridge = pci_find_device(PCI_VENDOR_ID_SI, PCI_DEVICE_ID_SI_5513,
				     (void *) NULL);
        if (isa_bridge == NULL) {
                printk_err("%s:%s Can not find ISA bridge\n", __FILE__, __LINE__);
                return 0;
        }
...
struct nic *sis900_probe(struct nic *nic, unsigned short *io_addrs, struct pci_dev *pci)
{
	...
	pci_write_config_byte(pci, 0x04, 0x07);
	...
	if (ret == 0) {
		printk_err("sis900_probe: Error MAC address not found\n");
		return NULL;
	}
