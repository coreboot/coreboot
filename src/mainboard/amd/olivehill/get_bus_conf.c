...
void get_bus_conf(void)
{
	...
	/************************************
	 * Configure the SATA port expander
	 ************************************/
	device_t sata_dev;
	u32 temp;
	...
	// read the VENDEV ID
	sata_dev = dev_find_slot( 0, PCI_DEVFN( 0x11, 0));

	// unlock the write-protect
	temp = pci_read_config32(sata_dev, 0x40);
	temp |= 0x01;
	pci_write_config32( sata_dev, 0x40, temp);

	// set the SATA AHCI mode to allow port expanders
	mem_ptr = (u32 *)(pci_read_config32(sata_dev, 0x24) & 0xFFFFFF00);
	*(mem_ptr + (0xFC>>2)) |= 0x1000;
