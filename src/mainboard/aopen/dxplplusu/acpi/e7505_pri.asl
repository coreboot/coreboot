        ...
        Device (MBRS)
        {
            ...
            Name (MSBF, ResourceTemplate ()
            {
                ...
                QWordMemory (ResourceProducer, PosDecode, MinFixed, MaxFixed, Prefetchable, ReadWrite,
                    0x0, 0x100000000, 0x100000000, 0x0, 0x0, ,, _Y1C, AddressRangeMemory, TypeStatic)
		...
		/* Local APIC */
            ...
            {
                CreateDWordField (MSBF, \_SB.MBRS._Y1C._MIN, MEML)
                CreateDWordField (MSBF, \_SB.MBRS._Y1C._MAX, MEMM)
                CreateDWordField (MSBF, \_SB.MBRS._Y1C._LEN, LELM)
                CreateDWordField (MSBF, 0x2A, LEHM)
                And (\_SB.PCI0.RLAR, 0x03FF, Local1)
                Increment (Local1)
                If (LGreater (Local1, 0x40))
                {
                    ShiftRight (Local1, 0x06, Local0)
                    ShiftLeft (Local1, 0x1A, Local1)
                    Subtract (Local0, 0x01, LEHM)
                    Store (Local1, LELM)
                }
