...
static void qemu_ports_used(struct device *dev, unsigned int idx,
...
//	int q35    = (nbid == 0x29c0);
	...
	qemu_ports_used(dev, idx++, 0x0510, 0x02, "firmware-config");
	qemu_ports_used(dev, idx++, 0x5658, 0x01, "vmware-port");
	if (i440fx) {
		qemu_ports_used(dev, idx++, 0xae00, 0x10, "pci-hotplug");
		qemu_ports_used(dev, idx++, 0xaf00, 0x20, "cpu-hotplug");
		qemu_ports_used(dev, idx++, 0xafe0, 0x04, "piix4-gpe0");
	}
	if (inb(CONFIG_CONSOLE_QEMU_DEBUGCON_PORT) == 0xe9) {
		qemu_ports_used(dev, idx++, CONFIG_CONSOLE_QEMU_DEBUGCON_PORT, 1,
				"debugcon");
	}
