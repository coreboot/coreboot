...
unsigned long acpi_fill_madt(unsigned long current)
{
	...
	/* IOAPIC */
	current += acpi_create_madt_ioapic((acpi_madt_ioapic_t *) current, 8, IO_APIC_ADDR, irq_start);
	irq_start += 24;
	current += acpi_create_madt_ioapic((acpi_madt_ioapic_t *) current, 9, IO_APIC_ADDR + 0x80000, irq_start);
	irq_start += 24;
