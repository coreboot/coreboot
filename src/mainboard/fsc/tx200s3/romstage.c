...
static void setup_gpio(void)
{
	...
	outl(0x00030300, DEFAULT_GPIOBASE + 0x38); /* GPIO_LVL2 */
}

static void i5000_lpc_config(void)
{
	pci_write_config16(PCI_DEV(0, 0x1f, 0), 0x82, 0x360f);
	pci_write_config16(PCI_DEV(0, 0x1f, 0), 0x80, 0x10);

	pci_write_config16(PCI_DEV(0, 0x1f, 0), 0x84, 0xe01);
	pci_write_config16(PCI_DEV(0, 0x1f, 0), 0x88, 0xca1);
}
...
void main(unsigned long bist)
{
	...
	pilot_enable_serial(PNP_DEV(0x4e, 2), 0x3f8);
	...
	device_t dev = PNP_DEV(0x4e, 0);
	pnp_enter_ext_func_mode(dev);
	pnp_set_logical_device(dev);
	pnp_set_enable(dev, 0);
	pnp_set_iobase(dev, 0x60, 0xf00);
	...
	pnp_set_logical_device(dev);
	pnp_set_enable(dev, 0);
	pnp_set_iobase(dev, 0x60, 0xe00);
	pnp_set_iobase(dev, 0x62, 0x200);
	pnp_set_iobase(dev, 0x64, 0x204);
	pnp_set_iobase(dev, 0x66, 0x208);
	pnp_set_enable(dev, 1);
	pnp_exit_ext_func_mode(dev);
	...
	pnp_set_logical_device(dev);
	pnp_set_enable(dev, 0);
	pnp_set_iobase(dev, 0x60, 0xca4);
	pnp_set_iobase(dev, 0x62, 0xca5);
	pnp_set_enable(dev, 1);
	pnp_exit_ext_func_mode(dev);
	...
	pnp_set_logical_device(dev);
	pnp_set_enable(dev, 0);
	pnp_set_iobase(dev, 0x60, 0xca2);
	pnp_set_iobase(dev, 0x62, 0xca3);
	pnp_set_enable(dev, 1);
	pnp_exit_ext_func_mode(dev);

	dev = PNP_DEV(0x4e, 0x0a);
	pnp_enter_ext_func_mode(dev);
	pnp_set_logical_device(dev);
	pnp_set_enable(dev, 0);
}
