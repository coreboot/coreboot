...
{
	uint8_t val = 0xbb;
	printk(BIOS_DEBUG, "%s: attempting to enable FET6\n", __func__);
	i2c_set_bus_num(TPS69050_BUS);
	i2c_read(TPS69050_ADDR, FET6_CTRL, 1, &val, sizeof(val));
	printk(BIOS_DEBUG, "%s: FET6_CTRL before: 0x%02x\n", __func__, val);
	val |= 1;
	i2c_write(TPS69050_ADDR, FET6_CTRL, 1, &val, sizeof(val));

	i2c_read(TPS69050_ADDR, FET6_CTRL, 1, &val, sizeof(val));
	printk(BIOS_DEBUG, "%s: FET6_CTRL after: 0x%02x\n", __func__, val);
...
//	return board_ldo_enable(1); /* Enable FET1, backlight */
	/* FIXME: implement more paranoid tps65090EXYNOS5250_DP1_ADDR fet routine */
//	ret = tps65090_fet_enable(1);
	printk(BIOS_DEBUG, "%s: attempting to enable FET1\n", __func__);
	i2c_set_bus_num(TPS69050_BUS);
	i2c_read(TPS69050_ADDR, FET1_CTRL, 1, &val, sizeof(val));
	printk(BIOS_DEBUG, "%s: FET1_CTRL before: 0x%02x\n", __func__, val);
	val |= 1;
	i2c_write(TPS69050_ADDR, FET1_CTRL, 1, &val, sizeof(val));
	udelay(LCD_T5_DELAY_MS * 1000);

	i2c_read(TPS69050_ADDR, FET1_CTRL, 1, &val, sizeof(val));
	printk(BIOS_DEBUG, "%s: FET1_CTRL after: 0x%02x\n", __func__, val);
}
...
static void mainboard_init(device_t dev)
{
	...
	/* FIXME: fix the braindead i2c crap leftover from u-boot */
	i2c_set_early_reg(0x12ca0000);
