...
static void board_setup_power(void)
{
	int error = 0;
...
static void board_setup_storage(void)
{
	/* MMC0: Fixed, 8 bit mode, connected with GPIO. */
	if (clock_set_mshci(PERIPH_ID_SDMMC0))
		print_crit("Failed to set clock for SDMMC0.\n");
	if (gpio_direction_output(MMC0_GPIO_PIN, 1)) {
		print_crit("Unable to power on SDMMC0.\n");
	}
...
static void board_setup_graphics(void)
{
...
static void board_setup_memory(struct mem_timings *mem, int is_resume)
{
...
static struct mem_timings *board_setup_clock(void)
{
