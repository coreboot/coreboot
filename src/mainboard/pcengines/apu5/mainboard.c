...
static void config_gpio_mux(void)
{
	...
	if (uart)
		uart->enabled = CONFIG_APU5_PINMUX_UART_C;
	if (gpio)
		gpio->enabled = CONFIG_APU5_PINMUX_GPIO0;

	uart = dev_find_slot_pnp(SIO_PORT, NCT5104D_SP4);
	gpio = dev_find_slot_pnp(SIO_PORT, NCT5104D_GPIO1);
	if (uart)
		uart->enabled = CONFIG_APU5_PINMUX_UART_D;
	if (gpio)
		gpio->enabled = CONFIG_APU5_PINMUX_GPIO1;
}
