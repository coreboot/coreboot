...
static void add_dev_list(struct pci_bus *bus, struct cdev *dev);

static void add_device(struct pci_bus *bus, struct cdev *dev)
{
	struct pci_dev *pdev;
	struct pci_bus *pbus;
	...
	pdev->sibling = bus->devices;
	bus->devices = pdev;


	/* Leave if we aren't a bus. */
	if (dev->children == &nodev)
		return;
	...
	pbus->sibling = bus->children;
	bus->children = pbus;
	pbus->self    = pdev;
	pbus->parent  = bus;

	add_dev_list(pbus, dev->children);
	return;
 no_mem:
