...
static long spd_enable_2channels(const struct mem_controller *ctrl, long dimm_mask)
{
	...
	static const uint8_t addresses[] = {
		...
		25,	/* *Cycle time at CAS Latnecy (CLX - 1.0) */
...
// well, there are socket 940 boards supported which obviously fail to compile with this
...
static struct spd_set_memclk_result spd_set_memclk(const struct mem_controller *ctrl, long dimm_mask)
{
	...
	/* if the next lower frequency gives a CL at least one whole cycle shorter, select that
	 * (see end of BKDG 4.1.1.1) */
