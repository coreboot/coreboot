...
static int i5000_read_spd_data(struct i5000_fbdimm *d)
{
	...
	if (spd_read_byte(d, FBDIMM_SPD_SDRAM_ADDRESSING, 1, &addr) ||
	    ...
	    spd_read_byte(d, cmd2data_addr, 1, &d->cmd2datanxt) ||
...
static int i5000_amb_preinit(struct i5000_fbdimm *d)
{
	...
	i5000_amb_smbus_write_config8(d, 1, AMB_CMD2DATANXT, d->cmd2datanxt);
...
static int i5000_amb_postinit(struct i5000_fbdimm *d)
{
	...
	i5000_amb_write_config16(d, 4, 0xb6, p16[3]);
	i5000_amb_write_config32(d, 4, 0xb8, p32[2]);
	i5000_amb_write_config16(d, 4, 0xbc, p16[6]);
...
static int i5000_dram_timing_init(struct i5000_fbd_setup *setup)
{
	...
	for(branch = 0; branch < I5000_MAX_BRANCH; branch++) {
		if (setup->branch[branch].used) {
			dev = setup->branch[branch].branchdev;
			mir |= 1 << branch;
			pci_mmio_write_config32(dev, I5000_DMIR0, 0x20000);
			pci_mmio_write_config32(dev, I5000_DMIR1, 0x20000);
			pci_mmio_write_config32(dev, I5000_DMIR2, 0x20000);
			pci_mmio_write_config32(dev, I5000_DMIR3, 0x20000);
			pci_mmio_write_config32(dev, I5000_DMIR4, 0x20000);
...
void i5000_fbdimm_init(void)
{
	...
	while (i5000_for_each_branch(&setup, i5000_branch_reset) ||
	...
	while (i5000_for_each_channel(&setup.branch[0], i5000_drive_test_patterns0) ||
	    i5000_for_each_channel(&setup.branch[1], i5000_drive_test_patterns0))
		printk(BIOS_INFO, "Channel training failed\n");
