...
static int i5000_train_channel_idle(struct i5000_fbd_channel *c)
{
	...
	for(i = 0; i < 4; i++) {
		if (c->dimm[i].present)
			i5000_amb_smbus_write_config32(c->dimm + i, 1, AMB_FBDSBCFGNXT, i ? 0x21b1b : 0x20b1b);
	}
...
static int i5000_setup_dmir(struct i5000_fbd_branch *b)
{
	...
	ranksize = b->channel[0].dimm[0].size << 7;

	if (b->channel[0].dimm[0].ranks == 2)
		ranksize <<= 1;
	...
	while(ranks) {
		...
		if (ranks >= 4) {
			...
			printk(BIOS_DEBUG, "ranks2\n");
			for(i = 7; set < 2 && i >= 0; i--) {
				if (rankmap & (1 << i)) {
				printk(BIOS_DEBUG, "%d set\n", i);
					rankmap &= ~(1 << i);
					ranks--;
					dmirval |= (i << (set * 3)) | (i << (6 + (set * 3)));
					set++;
				}
			}
		} else {
		...
		printk(BIOS_DEBUG, "a ranks %d DMIR%d: %08x\n", ranks, (dmir - I5000_DMIR0) >> 2,
		       dmirval);
	...
	for(; dmir <= I5000_DMIR4; dmir += 4) {
		printk(BIOS_DEBUG, "b DMIR%d: %08x\n", (dmir - I5000_DMIR0) >> 2, dmirval);
		pci_mmio_write_config32(dev, dmir, dmirval);
	}
	return rankoffset;
}
