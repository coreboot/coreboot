...
static int vx900_int15_handler_yabel(void)
{
	...
	case 0x5f01:
		...
		break;
	case 0x5f02:
	{	/* <- Without this block the compiler shits its pants */
		/* Boot device selection */
		u8 hw_opt = 1<<0; /* DVI not present, CRT present */
		u8 hdtv_conf2 = 1<<2; /* Connector type: R/G/B */
		u8 tv_conf2 = 1<<2; /* Only RGB */
		M.x86.R_EBX = (hdtv_conf2 << 16) | (tv_conf2 << 8) | hw_opt;
		u16 dev_word = 0x2383; /* CRT1/2, LCD1/2, HDMI1/2 */
		u8 tv_conf1 = 0; /* Default RGB NTSC */
		u8 hdtv_conf1 = 0x06; /* 1080P HDTV */
		M.x86.R_ECX= (hdtv_conf1 << 24) | (tv_conf1 << 16) | dev_word;
