...
static void dram_find_spds_ddr3(const dimm_layout *addr, dimm_info *dimm)
{
	...
	do {
		...
		if(dimm->dimm[i].dram_type != SPD_MEMORY_TYPE_SDRAM_DDR3) continue;
		dimms++;
...
static void dram_find_common_params(const dimm_info *dimms, ramctr_timing *ctrl)
{
	...
	for(i = 0; i < VX900_MAX_DIMM_SLOTS; i++)
	{
		...
		if(dimm->dram_type == SPD_MEMORY_TYPE_UNDEFINED) continue;
		valid_dimms++;
				       ...
				       rank_layout *ranks)
{
	...
	for(i = 0; i < VX900_MAX_DIMM_SLOTS; i ++)
	{
		...
		ranks->flags[i*2 +1] = ranks->flags[i*2] = dimms->dimm[i].flags;
				      ...
				      const rank_layout *ranks)
{
	...
	for(i = 0; i < VX900_MAX_MEM_RANKS; i++)
	{
		...
		const u8 ods = ranks->flags[i].rzq7_supported ?
			DDR3_MR1_ODS_RZQ7 : DDR3_MR1_ODS_RZQ6;


		printram("Using Hardware method\n");
		u8 swap = ranks->flags[i].pins_mirrored;
		if(swap) printram("Pins mirrored\n");
		printram(" Swap    : %x\n", swap);
		printram(" rtt_nom : %x\n", rtt_nom);
		printram(" ods     : %x\n", ods);
		printram(" rtt_wr  : %x\n", rtt_wr);
		vx900_dram_ddr3_do_hw_mrs(swap, rtt_nom, ods, rtt_wr, 0, 0);
...
static void vx900_dram_find_avg_delays(vx900_delay_calib *delays)
{
	...
	if(0)for(i = 0; i < 8; i++)
	{
		 ...
		 * We cast to u16 to prevent overflows due to the addition */
		delays->tx_dq[0].avg[i]  = (u8)(((u16)dq_low  + dq_high )/2);
		delays->tx_dqs[0].avg[i] = (u8)(((u16)dqs_low + dqs_high)/2);
	}

	for(i = 0; i < 8; i++)
	{
		u8 dq_avg  = (u8)(((u16)tx_dq_a->avg[i]  + tx_dq_b->avg[i] )/2);
		u8 dqs_avg = (u8)(((u16)tx_dqs_a->avg[i] + tx_dqs_b->avg[i])/2);
...
static void vx900_dram_calibrate_recieve_delays(vx900_delay_calib *delays)
{
	...
	for(;;)
	{
		 ...
		 * out how to get this calibration mechanism running*/
				   ...
				   const rank_layout *ranks)
{
	...
	{
		/* Do we have a valid DIMM? */
		u32 dimm_size = ranks->phys_rank_size[2*i]
			      + ranks->phys_rank_size[2*i + 1];
		if(dimm_size == 0)
			continue;
		/* Map the first rank of the DIMM to VR0 */
		vx900_map_pr_vr(2*i, 0);
	...
	if(dimm > 1)
	{
		...
		printram("Tx DQS:      "); dump_delay(delay_cal.tx_dqs[0].avg);
		printram("Tx DQ:       "); dump_delay(delay_cal.tx_dq[0].avg);
	}
...
static void vx900_dram_range(ramctr_timing *ctrl, rank_layout *ranks)
{
	...
	u32 tolmk = ramsize * ((1<<24) >> 10);
	tolmk = min(tolmk, TOLM_3_5G >> 10);
