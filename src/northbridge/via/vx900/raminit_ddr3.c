...
static u8 vx900_dram_check_calib_range(delay_range * dly, u8 window)
{
	size_t i;
	for(i = 0; i < 8; i++)
	{
		if(dly->high[i] - dly->low[i] < window)
			return -1;
		/* When our maximum value is lower than our max, both values
...
static void vx900_dram_calibrate_recieve_delays(vx900_delay_calib *delays)
{
	...
	for(;;)
	{
		 ...
		 * issue.
		 * I kept this code in because the long-term plan is to figure
		 * out how to get this calibration mehcanism running*/
			vx900_rx_capture_range_calib();
			vx900_read_delay_range(&(delays->rx_dq_cr),
					       CALIB_RxDQ_CR);
			dump_delay_range(delays->rx_dq_cr);

		} else {
			/*FIXME: Cheating with Rx CR setting\
			 * We need to either use Rx CR calibration
			 * or set up a table for the calibration */
			u8 *override = &(delays->rx_dq_cr.avg[0]);
			override[0] = 0x28; override[1] = 0x1c;
			override[2] = 0x28; override[3] = 0x28;
			override[4] = 0x2c; override[5] = 0x30;
			override[6] = 0x30; override[7] = 0x34;
			printram("Bypassing RxCR 78-7f calibration with:\n");
			dump_delay(delays->rx_dq_cr.avg);
		}
		...
		vx900_write_0x78_0x7f(delays->rx_dq_cr.avg);

		/************* RxDQS *************/
		vx900_rx_dqs_delay_calib();
		vx900_read_delay_range(&(delays->rx_dqs), CALIB_RxDQS);
		vx900_rxdqs_adjust(&(delays->rx_dqs));

		u8 howd_we_do = vx900_dram_check_calib_range(&(delays->rx_dqs),
					     DQSI_THRESHOLD);
		if(howd_we_do != DELAY_RANGE_GOOD)
			continue;
		...
		vx900_delay_calib_mode_select(CALIB_RxDQS, CALIB_MANUAL);
		vx900_write_0x78_0x7f(delays->rx_dqs.avg);
		break;
	}
}

static void vx900_dram_calibrate_transmit_delays(vx900_delay_calib *delays)
{
	...
	for(;;)
	{
		...
		vx900_read_delay_range(&(delays->tx_dqs), CALIB_TxDQS);
		...
		vx900_read_delay_range(&(delays->tx_dq), CALIB_TxDQ);

		howd_we_do = vx900_dram_check_calib_range(&(delays->rx_dqs),
							     DQO_THRESHOLD);
		if(howd_we_do != DELAY_RANGE_GOOD)
			continue;
		...
		vx900_write_0x78_0x7f(delays->tx_dqs.avg);
		vx900_delay_calib_mode_select(CALIB_TxDQ, CALIB_MANUAL);
		vx900_write_0x78_0x7f(delays->tx_dq.avg);
				   ...
				   const rank_layout *ranks)
{
	...
	/* Enable multiple page mode for transmit delays */
	...
	for(i = 0; i < VX900_MAX_DIMM_SLOTS; i++)
	{
		...
		/* Map the first rank of the DIMM to VR0 */
		vx900_map_pr_vr(2*i, 0);
		vx900_dram_calibrate_transmit_delays(&delay_cal);
	}
...
static void vx900_dram_write_final_config(ramctr_timing *ctrl)
{
	...
	pci_mod_config8(MCU, 0x69, 0, 0xc5);
