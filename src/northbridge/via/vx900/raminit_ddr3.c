...
/*
static void vx900_dirty_read_test(u16 addr)
{
	const u8 pattern = 0x5a;
	u8 even = pattern;
	u8 odd;
	do {
		odd = ~even;
		printram("-%.2x-%.2x",even, odd);
		even >>= 1;
	} while (even != 0);
	printram("\n");
	even = 0xa5;
	...
	addr &= ~0x000f;
	pci_write_config16(MCU, 0x8c, addr);
	pci_mod_config8(MCU, 0x8c, 0, 0x02);
	udelay(100);

	size_t i=0;
	for(i = 0; i < 0x40; i++)
	{
		u32 maddr = ((u32)addr) << 20;
		maddr += (i<<3);
		printram("Readin at 0x%.8x : %.8x - %.8x\n",
			 maddr, volatile_read(maddr), volatile_read(maddr + 4));
	}
}*/
...
#define USE_HW_INIT_SEQUENCE 0
		 ...
		 * It seems to work even if we don't send the long ZQ */
...
static void vx900_rx_dqs_delay_calib(void)
{
	...
	pci_write_config8(MCU, 0x78, ref_cnt);
