...
#define Rank3_ODT				3
/*
 * This is the table that tells us which ODT pin to map to which rank.
 *
...
static const u8 odt_lookup_table[][2] = {
	...
	{ 0x01, (Rank3_ODT << 6) | (Rank2_ODT << 4) | (Rank1_ODT << 2) | Rank0_ODT},
	{ 0x03, (Rank3_ODT << 6) | (Rank2_ODT << 4) | (Rank0_ODT << 2) | Rank1_ODT},
	{ 0x04, (Rank3_ODT << 6) | (Rank2_ODT << 4) | (Rank1_ODT << 2) | Rank0_ODT},
	...
	{ 0x0f, (Rank0_ODT << 6) | (Rank0_ODT << 4) | (Rank2_ODT << 2) | Rank2_ODT},
	{ 0, 0},
};
...
static void vx900_dram_driving_ctrl(const dimm_info * dimm)
{
	...
	for (i = 0; ; i++)
	{
		...
		val = odt_lookup_table[i][1];
		break;
	}

	printram("Mapping rank mask %x to ODT entry %.2x\n", rank_mask, val);
	pci_write_config8(MCU, 0x9c, val);
				      ...
				      const rank_layout * ranks)
{
	...
	for (i = 0; i < VX900_MAX_MEM_RANKS; i++) {
		...
		if (0)
			...
			vx900_dram_ddr3_do_sw_mrs(pinswap, rtt_nom, ods, rtt_wr, 0, 0);
