...
{
	static struct rsc_drv *drv = NULL;
	...
	if (!drv)
		drv = xzalloc(sizeof(struct rsc_drv));

	if (!drv) {
		printk(BIOS_INFO, "RPMH RSC: Failed to allocate memory for rsc_drv\n");
		return -1;
	}
	...
	non_batch_cache = xzalloc(sizeof(struct cache_req) * CMD_DB_MAX_RESOURCES);
	if (!non_batch_cache) {
		printk(BIOS_INFO, "RPMH RSC: Failed to allocate memory for non_batch_cache\n");
		free(drv);
		drv = NULL;
		return -1;
	}
