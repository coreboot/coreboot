			   ...
			   u32 addr, size_t len, const void *buf)
{
	...
	if (addr + len > flash->size) {
		printk (BIOS_ERR, "Attempt to write %x-%x which is out of chip\n",
			addr, addr+len);
		return -1;
	}
	...
	printk(BIOS_DEBUG, "SF: Successfully written %zu bytes @ %#x\n", addr - start, start);
	return 0;
}
		...
		unsigned int max_hz, unsigned int spi_mode)
{
	...
	if (cntlr.revision != 9) {
		printk(BIOS_WARNING, "SF: Unsupported revision %x\n",
			cntlr.revision);
		return NULL;
	}
