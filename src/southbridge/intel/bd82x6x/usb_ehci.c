...
static void usb_ehci_init(struct device *dev)
{
	...
	pci_write_config32 (dev, 0x68, 0x00010001);
...
static void usb_ehci_set_resources(struct device *dev)
{
  ...
  if (PCI_SLOT(dev->path.pci.devfn) == 0x1a)
	...
	if (!res) 
	  printk (BIOS_ERR, "alive "__FILE__ ":%d: resource not found\n", __LINE__);	    
	else
	  printk (BIOS_ERR, "alive "__FILE__ ":%d: resource %x\n", __LINE__, (unsigned)res->base); 
	base = res->base;
	printk (BIOS_ERR, "alive "__FILE__ ":%d\n", __LINE__);
