...
static void sata_enable_ahci_mmap(struct device *const dev, const config_t *const config,
				  const int is_mobile)
{
	int i;
	u32 reg32;
	...
	/* Lock R/WO bits in Port command registers.
	   ICH8-M has 3 SATA ports, ICH8 desktop has up to 6. */
	const int num_ports = is_mobile ? 3 : 6;
...
static void sata_init(struct device *const dev)
{
	...
	const struct southbridge_intel_i82801hx_config *const config = dev->chip_info;

	const u16 devid = pci_read_config16(dev, PCI_DEVICE_ID);
	const int is_mobile = (devid == PCI_DID_INTEL_82801HBM_SATA) ||
			      (devid == PCI_DID_INTEL_82801HBM_SATA_AHCI) ||
			      (devid == PCI_DID_INTEL_82801HBM_SATA_RAID);

	/* ICH8-M has 3 SATA ports, ICH8 desktop has up to 6. */
	const u8 port_mask = is_mobile ? 0x07 : 0x3f;

	printk(BIOS_DEBUG, "i82801hx_sata: initializing...\n");

	if (config == NULL) {
		printk(BIOS_ERR, "i82801hx_sata: device not in devicetree.cb!\n");
		return;
	}
	...
	reg16 &= ~0x3f;
	reg16 |= (1 << 15) | ((sata_mode == 0) ? port_mask : config->sata_port_map);
	pci_write_config16(dev, 0x92, reg16);

	/* SATA clock settings */
	u32 sclkcg = 0;
	if (config->sata_clock_request && !(inb(DEFAULT_GPIOBASE + 0x30) & (1 << (35 - 32))))
	...
	if (is_mobile && config->sata_traffic_monitor) {
		...
		if (((pci_read_config8(lpc_dev, D31F0_C5_ENABLE) >> 3) & 3) == 3) {
...
static void sata_enable(struct device *dev)
{
	...
	/* ICH8 SATA MAP register is 8 bits wide. */
	pci_write_config8(dev, 0x90, map & 0xff);
}
...
static const unsigned short pci_device_ids[] = {
	PCI_DID_INTEL_82801HB_SATA1,
	PCI_DID_INTEL_82801HB_SATA_AHCI_6,
	PCI_DID_INTEL_82801HB_SATA_RAID,
	PCI_DID_INTEL_82801HB_SATA_AHCI_4,
	PCI_DID_INTEL_82801HB_SATA2,
	PCI_DID_INTEL_82801HBM_SATA,
	PCI_DID_INTEL_82801HBM_SATA_AHCI,
	PCI_DID_INTEL_82801HBM_SATA_RAID,
	0,
};

static const struct pci_driver ich8_sata __pci_driver = {
	.ops		= &sata_ops,
