...
#endif
#if 0
static void DramcCmdBusTrainingPostProcess(DRAMC_CTX_T *p)
{
...
static U16 DramcMiockJmeter(DRAMC_CTX_T *p)
{
...
static void DutyScan_Offset_Calibration(DRAMC_CTX_T *p, U8 *u1OfB0, U8 *u1OfB1, U8 *u1OfCA)
{
    ...
    for(i=0; i<15; i++)
    {
        ...
        //mcSHOW_DBG_MSG2(("   %d    \t%d\t%d\t%d\n", DutyScan_Offset_Convert(i), read_val_b0, read_val_b1, read_val_ca)); [FOR_CHROMEOS]
...
static DRAM_STATUS_T DutyScan_Calibration_Flow(DRAMC_CTX_T *p, U8 k_type, U8 ft_test_type)
{
    ...
    U32 ucperiod_duty_averige=0, ucStart_period_duty_averige[DQS_BYTE_NUMBER]={0,0}, ucFinal_period_duty_averige[DQS_BYTE_NUMBER]={0,0}, ucmost_approach_50_percent=0xffffffff;
    //U32 ucperiod_duty_averige_past=0, ;
...
//fra    U8 *str_who_am_I=str_clk_duty;
...
#if FOR_DV_SIMULATION_USED
    scinner_duty_ofst_step = DUTY_OFFSET_STEP * 2;
#else
    ...
    for(ucdqs_i=0; ucdqs_i<ucdqs_i_count; ucdqs_i++)
    {
        ...
        if ((scFinal_clk_delay_cell[ucdqs_i]>8)||(scFinal_clk_delay_cell[ucdqs_i]<-8))
        {
    ...
    *GPIO_GPIO_DOUT0  |= 0x04000000; //PAD_PWM_0,      // Lafi_te / round done
    ...
    {
        // backup final values
        gcFinal_K_DQS_delay_cell[0] = scFinal_clk_delay_cell[0];
        gcFinal_K_DQS_delay_cell[1] = scFinal_clk_delay_cell[1];
    ...
    {
        // backup final values
        gcFinal_K_WCK_delay_cell[0] = scFinal_clk_delay_cell[0];
        gcFinal_K_WCK_delay_cell[1] = scFinal_clk_delay_cell[1];
    ...
    if (k_type == DutyScan_Calibration_K_CLK)
    {
        ...
        // backup final values
        gcFinal_K_CLK_delay_cell[0] = scFinal_clk_delay_cell[0];
        gcFinal_K_CLK_delay_cell[1] = scFinal_clk_delay_cell[1];
    ...
    {
        // backup final values
        gcFinal_K_DQ_delay_cell[0] = scFinal_clk_delay_cell[0];
        gcFinal_K_DQ_delay_cell[1] = scFinal_clk_delay_cell[1];
    ...
    {
        // backup final values
        gcFinal_K_DQM_delay_cell[0] = scFinal_clk_delay_cell[0];
        gcFinal_K_DQM_delay_cell[1] = scFinal_clk_delay_cell[1];
