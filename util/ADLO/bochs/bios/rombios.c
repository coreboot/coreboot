...
static char bios_info_string[] = "BOCHS BIOS VER:1.79";
...
ASM_END
  ...
  void
int1a_function(regs, es, ds, iret_addr)
  pusha_regs_t regs; // regs pushed from PUSHA instruction
  Bit16u es;
  Bit16u ds; // previous DS:, DS set to 0x0000 by asm wrapper
  iret_addr_t  iret_addr; // CS,IP,Flags pushed from original INT call
...
/***********************************************************/
      } else if (regs.u.r8.al == 0x01) {
        BX_INFO("PCI BIOS function 0x%02x\n", regs.u.r8.al);

	regs.u.r8.ah   = 0x00;		// 0x0 = works
	regs.u.r8.al   = 0x01;		// desc (0x11 ?????)
	regs.u.r8.bh   = 0x02;		// major
	regs.u.r8.bl   = 0x10;		// minor
	regs.u.r8.cl   = 0x01;		// count

//	regs.u.r32.edx = 0x20494350; 	// " ICP"
//	regs.u.r32.edi = 0x00000000; 	// entry

        ClearCF(iret_addr.flags); // OK
        break;

/***********************************************************/
      } else if (regs.u.r8.al == 0x0E) {
	oursize = read_word(0xF000,0xFD00+6);
	size = read_word(regs.u.r16.dx,regs.u.r16.di);
//	size = read_word(es,regs.u.r16.di);
//	size = read_word(0x16f3,regs.u.r16.di);
        BX_INFO("PCI BIOS function 0x%02x es:%x di:%x s:%x\n", regs.u.r8.al,es,regs.u.r16.di,size);

	if (size < 0x70 ) {
		// ix this 89 or 59 ???
		regs.u.r8.ah   = 0x89;		// not enough space
		write_word(regs.u.r16.dx,regs.u.r16.di, oursize);
//		write_word(regs.u.r16.dx,regs.u.r16.di, 0x0);
	       	SetCF(iret_addr.flags);
		break;
	}
...
//	memcpyb(es,regs.u.r16.di,0xF000,pirq_table,0x0070);
/* */
ASM_START
...
*/
      ...
      mov  cx, [pirq_table+6] ; count

      mov  ax, #0xF000 ; ssegment
      mov  ds, ax

      mov  ax, #pirq_table ; soffset
      mov  si, ax
