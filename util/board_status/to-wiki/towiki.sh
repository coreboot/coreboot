...
for vendor_board_dir in "$COREBOOT_DIR"/src/mainboard/*/* ; do
    ...
    northbridge_nice="$(echo "$northbridge"|sed 's,_, ,g;s,INTEL,Intel®,g;s,AMD_AGESA_FAMILY\(.*\),AMD Family \1h (AGESA),g;s,AMD_FAMILY\(.*\),AMD Family \1h,g;';s,AMD_AMDFAM\(.*\),AMD Family \1h,g;')"
