...
struct param {
	...
	uint32_t rom_bootaddress_assigned;
	uint32_t rom_headeraddress;
	uint32_t rom_headeradress_assigned;
...
static int cbfs_add_file(const char *rom_filename,
	...
	do {
		...
		if (cbfs_add_entry(&image, rom_name, rom_type, 0, &buffer) != 0)
			break;
...
static int cbfstool_create(void)
{
	...
	// Setup default boot address and header address.
	if (!param.rom_bootaddress_assigned) {
		// put boot block before end of ROM.
		param.rom_bootaddress = param.rom_size - bootblock.size;
		DEBUG("bootblock in end of ROM.\n");
	}
	if (!param.rom_headeradress_assigned) {
		...
		if (bootblock.size >= sizeof(uint32_t)) {
			...
			uint32_t ptr = param.rom_headeraddress - param.rom_size;
			uint32_t *sig = (uint32_t*)(bootblock.data +
						    bootblock.size -
						    sizeof(ptr));
			*sig = ptr;
			DEBUG("CBFS header address set in end of bootblock.\n");
		}
	}
...
static int cbfstool_locate(void)
{
	struct cbfs_image image;
	...
	if (buffer_from_file(&buffer, param.rom_filename) != 0) {
		fprintf(stderr, "E: Cannot load %s.\n", param.rom_filename);
		cbfs_image_delete(&image);
		return 1;
	}
	if (cbfs_get_entry(&image, param.rom_name)) {
	...
	if (cbfs_add_entry(&image, param.rom_name, CBFS_COMPONENT_RAW,
			   param.rom_alignment, &buffer) != 0) {
		// Legacy output.
		printf("0\n");
		buffer_delete(&buffer);
		cbfs_image_delete(&image);
	...
	entry = cbfs_get_entry(&image, param.rom_name);
...
static const struct command commands[] = {
	...
	{"locate", "vf:n:a:h?", cbfstool_locate},
	...
	for (i = 0; i < ARRAY_SIZE(commands); i++) {
		...
		while (1) {
			...
			case 'A':
				...
				param.rom_bootaddress_assigned = 1;
				break;
			case 'H':
				param.rom_headeraddress = strtoul(
						optarg, NULL, 0);
				param.rom_headeradress_assigned = 1;
				break;
			case 'a':
