...
static int cbfs_remove(void)
{
	...
	if (load_cbfs_image(&image, param.cbfs_name) != 0) {
		fprintf(stderr, "E: Could not load ROM image '%s'.\n",
			param.cbfs_name);
		return 1;
	}
	...
	do {
		...
		if (write_cbfs_image(&image, param.cbfs_name) != 0)
			break;
		result = 0;
	} while (0);
...
#if 0
	return create_cbfs_image(param.cbfs_name, param.rom_size, param.rom_bootblock,
						param.rom_alignment, param.rom_offset);
#endif
	delete_cbfs_image(&image);
	return 0;
