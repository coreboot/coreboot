...
void recalculate_rom_geometry(void *romarea)
{
	if (find_master_header(romarea, romsize)) {
		printf("Cannot find master header\n");
		exit(1);
	}

	switch (master_header->architecture) {
	...
	case CBFS_ARCHITECTURE_UNKNOWN: /* For old CBFS archives */
		...
		const char *bootblock, uint32_t align, uint32_t offs)
{
	...
	case ARCH_ARMV7:
		...
		/* FIXME: assume little-endian for now */
		master_header->architecture = CBFS_ARCHITECTURE_ARMV7;
