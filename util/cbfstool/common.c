...
int cbfs_add_entry(struct cbfs_image *image, const char *name,
	...
	DEBUG("(trying to merge empty sections...)\n");
...
int cbfs_remove_entry(struct cbfs_image *image, const char *name) {
	...
	memset(CBFS_SUBHEADER(entry), -1, ntohl(entry->len));
	return 0;
}
...
int cbfs_merge_empty_entry(struct cbfs_image *image, struct cbfs_file *entry,
	...
	// Now, we find two deleted/empty sections; try to merge now.
	...
	// TODO clear name
	memset(CBFS_SUBHEADER(entry), -1, newlen);
	// TODO we should try again until no more sections are found...
	return 0;
}
...
int cbfs_create_entry(struct cbfs_image *image, struct cbfs_file *entry,
	...
	memset(CBFS_SUBHEADER(entry), -1, len);
	return 0;
}
