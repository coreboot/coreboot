...
int cbfs_add_entry(struct cbfs_image *image, struct buffer *buffer) {
	struct cbfs_file *entry = cbfs_find_first_entry(image);
	uint32_t type, len;
	size_t offset;
	assert(entry);

	while (entry && cbfs_is_valid_entry(entry)) {
		...
		offset = cbfs_get_entry_offset(image, entry);
		len = ntohl(entry->len);

		DEBUG("cbfs_add_entry: %s at 0x%zx, %d bytes\n",
		      get_cbfs_entry_type_name(type), offset, len);
...
int cbfs_remove_entry(struct cbfs_image *image, const char *name) {
	...
	DEBUG("cbfs_remove_entry: Removed %s @ 0x%x\n",
	      name, cbfs_get_entry_offset(image, entry));
...
int cbfs_export_entry(struct cbfs_image *image, const char *entry_name,
	...
	LOG("Found file %.30s at 0x%x, type %.12s, size %d\n",
	    entry_name, cbfs_get_entry_offset(image, entry),
...
int cbfs_print_entry_info(struct cbfs_image *image, struct cbfs_file *entry,
	...
	printf("%-30s 0x%-8x %-12s %d\n",
	       *name ? name : "(empty)",
	       cbfs_get_entry_offset(image, entry),
	       get_cbfs_entry_type_name(ntohl(entry->type)),
	       ntohl(entry->len));
...
struct cbfs_file *cbfs_find_next_entry(struct cbfs_image *image,
	...
	int32_t offset = cbfs_get_entry_offset(image, entry);
	int align = ntohl(image->header->align);
	assert(entry && cbfs_is_valid_entry(entry));
	offset += ntohl(entry->offset) + ntohl(entry->len);
	if (offset % align)
		offset += (align - offset % align);
	return (struct cbfs_file*)(image->buffer.data + offset);
}

int32_t cbfs_get_entry_offset(struct cbfs_image *image, struct cbfs_file *entry) {
...
uint32_t cbfs__location(const char *romfile, uint32_t filesize,
