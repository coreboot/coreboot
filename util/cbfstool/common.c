...
/* Tries to bind entry data (CBFS_SUBHEADER) at given offset. */
int cbfs_bind_entry_at(struct cbfs_image *image,
		       struct cbfs_file *entry,
		       uint32_t offset) {
	...
	// If there are plenty of space, split entry; otherwise use offset.
...
int cbfs_add_entry(struct cbfs_image *image, struct buffer *buffer,
	...
	uint32_t header_size, need_size, new_size, min_size;

	header_size = (sizeof(*entry) + align_up(strlen(name) + 1,
						 CBFS_FILENAME_ALIGN));
	min_size = (sizeof(*entry) + align_up(1,CBFS_FILENAME_ALIGN));

	need_size = header_size + buffer->size;
	DEBUG("cbfs_add_entry('%s'@0x%x) => need_size = %u+%u=%u\n",
	...
	for (entry = cbfs_find_first_entry(image);
		...
		DEBUG("cbfs_add_entry: space at 0x%x, %d bytes\n",
		      addr, addr_next - addr);
		if (addr + need_size > addr_next)
			continue;

		if (content_offset && addr + header_size != content_offset) {
			if (content_offset > addr_next)
				break;
			if (content_offset < addr)
				continue;
			// ok, we're in right entry.
			DEBUG("found section 0x%x for content.\n", addr);
			if (data_offset < addr + header_size) {
				ERROR("Not enough space for header.\n");
				break;
			}
			if (data_offset + size > addr_next) {
				ERROR("Not enough space for content.\n");
				break;
			}
			if (data_offset - addr - header_size > min_size) {
				// split.
			}
		}

		// Should we create an empty partition before target offset?
		if (offset && addr + header_len != offset) {
			if (offset + size >= addr + capacity) {
				ERROR("");
			}
			DEBUG("Bind entry for data at 0x%x\n", addr);
			if (cbfs_bind_entry_at(image, entry, offset) != 0) {
				ERROR("Can't bind empty entry for 0x%x\n",
				      offset);
				return -1;
			}
			continue;
