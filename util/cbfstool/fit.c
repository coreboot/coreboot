...
/* Warning: This code assumes it is being executed on a little endian machine.*/
...
static inline void *rom_buffer_pointer(struct rom_image *image, int offset)
{
	return &image->rom[offset];
}
...
static struct fit_table *locate_fit_table(struct rom_image *image)
{
	struct fit_table *table;
	uint32_t *fit_pointer;

	fit_pointer = rom_buffer_pointer(image,
                              ptr_to_offset(image->size, FIT_POINTER_LOCATION));
	...
	table = rom_buffer_pointer(image,
	                           ptr_to_offset(image->size, *fit_pointer));
...
static void add_microcodde_entries(struct rom_image *image,
                                   ...
                                   struct microcode_entry *mcus, int num_mcus)
{
	...
	for (i = 0; i < num_mcus; i++) {
		...
		entry->address = offset_to_ptr(image->size, mcu->offset);
...
static int parse_microcode_blob(struct rom_image *image,
                                ...
                                struct microcode_entry *mcus, int *total_mcus)
{
	...
	current_offset = (int)((char *)mcode_file - image->rom);
	...
	while (file_length > sizeof(struct microcode_header))
	{
		...
		/* FIXME: Should the chcksum be validated? */
	...
	/* Update how many we found. */
	*total_mcus = num_mcus;

	return 0;
}

int fit_update_table(void *rom, uint32_t romsize, int empty_entries,
                     const char *microcode_blob_name)
{
	...
	if (!fit) {
		fprintf(stderr, "FIT not found.\n");
		return 1;
	}

	mcode_file = cbfs_find_file(microcode_blob_name);
	if (!mcode_file) {
		fprintf(stderr, "File '%s' not found in cbfs\n",
		        microcode_blob_name);
		return 1;
	}
	...
	if (!mcus) {
		fprintf(stderr, "Couldn't allocate memory for MCU entries.\n");
		return 1;
	}

	if (parse_microcode_blob(&image, mcode_file, mcus, &empty_entries)) {
		fprintf(stderr, "Couldn't parse microcode blob.\n");
		ret = 1;
		goto out;
	}
